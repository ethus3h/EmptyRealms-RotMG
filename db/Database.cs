...
namespace db {
    public partial class Database: IDisposable {
        MySqlConnection con;
        public Database() {
            con = new MySqlConnection("Server=127.0.0.1;Database=emptyrealms;uid=root;password=");
            con.Open();
        }
...
SELECT fame FROM stats WHERE accId=@accId;";
        ...
        public void ReadStats(Account acc) {
            ...
            if (acc.Stats.ClassStates.Count > 0) acc.Stats.BestCharFame = acc.Stats.ClassStates.Max(_ => _.BestFame);
...
SELECT MAX(chestId) FROM vaults WHERE accId = @accId;";
        ...
        public Char LoadCharacter(Account acc, int charId) {
            ...
            using(var rdr = cmd.ExecuteReader()) {
                ...
                if (!string.IsNullOrEmpty(ret.PCStats)) ret.FameStats.Read(ZlibStream.UncompressBuffer(Convert.FromBase64String(ret.PCStats.Replace('-', '+').Replace('_', '/'))));
                return ret;
            }
        }
...
VALUES(@accId, @chrId, @name, @objType, @tex1, @tex2, @items, @fame, @fameStats, @totalFame, @firstBorn, @killer);";
        ...
        public string GetCurrentVer() {
            ...
            cmd.CommandText = "SELECT cliVersion FROM clientnews ORDER BY date DESC LIMIT 1";
