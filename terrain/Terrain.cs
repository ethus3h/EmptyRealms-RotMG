...
namespace terrain
{
    class Terrain
    {
            ...
            using(Graphics g = Graphics.FromImage(map)) {
                foreach(var poly in polys) {
                    ...
                    for (int j = 0; j < poly.Nodes.Length; j++) {
                        ...
                        g.DrawLine(Pens.White, (float)(prev.X + 1) / 2 * Size, (float)(prev.Y + 1) / 2 * Size, (float)(curr.X + 1) / 2 * Size, (float)(curr.Y + 1) / 2 * Size);
                    }
                }
                if (plot != null) foreach(var i in plot)
        ...
        static int MinDistToMapEdge(PlanarGraph graph, Node n, int limit)
        {
            ...
            {
                var state = stack.Pop();
                if (state.Item2.Coordinate.X == 0 || state.Item2.Coordinate.X == Size || state.Item2.Coordinate.Y == 0 || state.Item2.Coordinate.Y == Size)
                {
                    if (state.Item1 < ret) ret = state.Item1;
                    if (ret == 0) return 0;
                ...
                foreach (var i in state.Item2.Edges)
                {
                    ...
                    if (!visited.Contains(node)) stack.Push(new Tuple<int, Node>(state.Item1 + 1, node));
                }
        ...
        static TerrainTile[,] CreateTerrain(int seed, PolygonMap map) {
            ...
            foreach(var poly in map.Polygons.Where(_ => !_.IsWater)) {
                ...
                poly.Nodes.SelectMany(_ => {
                    ...
                    (poly.Nodes[0].X + 1) / 2 * Size, (poly.Nodes[0].Y + 1) / 2 * Size
                }).ToArray(),
                new TerrainTile() {
                    PolygonId = poly.Id,
            ...
            foreach(var i in rivers) {
                for (int j = 1; j < i.Length; j++) {
                    ...
                    int count;
                    if (edges.TryGetValue(edge, out count)) count++;
                    else count = 1;
                    edges[edge] = count;
                }
            }
            foreach(var i in edges) {
                ...
                (i.Key.Item1.X + 1) / 2 * Size, (i.Key.Item1.Y + 1) / 2 * Size, (i.Key.Item2.X + 1) / 2 * Size, (i.Key.Item2.Y + 1) / 2 * Size,
                t => {
