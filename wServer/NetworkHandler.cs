...
namespace wServer {
    class NetworkHandler {
        ...
        public void BeginHandling() {
            ...
            if (!skt.ReceiveAsync(receive)) IOCompleted(this, receive);
        }
        ...
        void IOCompleted(object sender, SocketAsyncEventArgs e) {
            try {
                ...
                do {
                    ...
                    if (e.LastOperation == SocketAsyncOperation.Receive) {
                        switch (receiveState) {
                            case ReceiveState.ReceivingHdr:
                                ...
                                var len = (e.UserToken as ReceiveToken).Length = IPAddress.NetworkToHostOrder(BitConverter.ToInt32(e.Buffer, 0)) - 5;
                                if (len < 0 || len > BUFFER_SIZE) throw new InternalBufferOverflowException();
                    ...
                    } else throw new InvalidOperationException(e.LastOperation.ToString());
                } while (repeat);
            } catch (Exception ex) {
                OnError(ex);
            }
        ...
        bool CanSendPacket(SocketAsyncEventArgs e, bool ignoreSending) {
            lock(sendLock) {
                if (sendState == SendState.Ready || (!ignoreSending && sendState == SendState.Sending)) return false;
                Packet packet;
        ...
        public void SendPacket(Packet pkt) {
            try {
                ...
                if (CanSendPacket(send, false)) {
                    ...
                    if (!skt.SendAsync(send)) IOCompleted(this, send);
                }
            } catch {
