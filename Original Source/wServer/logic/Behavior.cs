...
namespace wServer.logic
{
    ...
    public abstract class BehaviorBase
    {
        ...
        protected Entity GetNearestEntity(ref float dist, short? objType)   //Null for player
        {
            ...
            else if (objType == 0) //NPC detection
            {
                var colMap = Host.Self.ObjectDesc.Enemy ? Host.Self.Owner.PlayersCollision : Host.Self.Owner.EnemiesCollision;
                foreach (var i in colMap.HitTest(Host.Self.X, Host.Self.Y, dist))
                {
                    if (i is IPlayer && !(i as IPlayer).IsVisibleToEnemy() && !Host.StateStorage.ContainsKey(MAGIC_EYE_KEY)) continue;
                    if (!(i is Enemy) || (i as Enemy).ObjectDesc == null || (i as Enemy).ObjectDesc.Enemy == Host.Self.ObjectDesc.Enemy) continue;
                    var d = Dist(i, Host.Self);
                    if (d < dist)
                    {
                        dist = d;
            ...
            else if (objType == 1) //Projectile detection
            {
                foreach (var i in Host.Self.Owner.ProjectilesCollision.HitTest(Host.Self.X, Host.Self.Y, dist))
                {
                    Projectile proj = i as Projectile;
                    try { if (proj.ProjectileOwner.Self.ObjectDesc.Enemy == Host.Self.ObjectDesc.Enemy) continue; }
                    catch { continue; }
                    var d = Dist(i, Host.Self);
                    if (d < dist)
                    {
        ...
        protected IEnumerable<Entity> GetNearestEntities(float dist, short? objType)   //Null for player
        {
            ...
            if (objType == null)
                ...
                {
                    Projectile proj = i as Projectile;
                    if (proj.ProjectileOwner == null) continue;
                    if (proj.ProjectileOwner.Self.ObjectDesc.Enemy == Host.Self.ObjectDesc.Enemy) continue;
                    var d = Dist(i, Host.Self);
                    if (d < dist)
                        yield return i;
                }
    ...
    public abstract class ConditionalBehavior : BehaviorBase
    {
        ...
        protected virtual void BehaveCore(BehaviorCondition cond, RealmTime? time, object state, Object[] args) { }
    ...
    class EntityGreaterThan : Behavior
    {
        float radius;
        int count;
        short? objType;
        private EntityGreaterThan(float radius, int count, short? objType)
        {
            this.radius = radius;
            this.count = count;
            this.objType = objType;
        }
        static readonly Dictionary<Tuple<float, int, short?>, EntityGreaterThan> instances = new Dictionary<Tuple<float, int, short?>, EntityGreaterThan>();
        public static EntityGreaterThan Instance(float radius, int count, short? objType)
        {
            var key = new Tuple<float, int, short?>(radius, count, objType);
            EntityGreaterThan ret;
            if (!instances.TryGetValue(key, out ret))
                ret = instances[key] = new EntityGreaterThan(radius, count, objType);
            return ret;
        }
