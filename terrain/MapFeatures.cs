    ...
    class MapFeatures
    {
        ...
        static MapEdge SelectDownhill(MapNode node)
        {
            ...
            foreach (var e in node.Edges)
                if (e.To.DistanceToCoast < dist) ret = e;
            return ret;
        }

        public IEnumerable<MapNode[]> GenerateRivers() {
            int c = 0;
            var eligibleRivers = map.Polygons.SelectMany(_ => _.Nodes)
        ...
        public IEnumerable<Coordinate[]> GenerateRoads() {
            var heights = map.Polygons.Select(_ => _.DistanceToCoast.Value)
                .Distinct()
                .OrderBy(_ => _).ToArray();
            ...
            if (i.IsOcean) queue.Enqueue(i);
            do {
                MapPolygon n = queue.Dequeue();
                foreach(var i in n.Neighbour) {
                    int newLevel;
                    if (!centerContour.TryGetValue(n, out newLevel)) newLevel = 0;

                    while (i.DistanceToCoast > roadHeights[newLevel])
                    newLevel++;

                    int iLevel;
                    if (!centerContour.TryGetValue(i, out iLevel)) iLevel = int.MaxValue;
            ...
            foreach(var i in map.Polygons.SelectMany(_ => _.Nodes).SelectMany(_ => _.Edges).Distinct()) {
                if (cornerContour[i.From] < cornerContour[i.To]) {
                    ...
                    (i.From.X + i.To.X) / 2, (i.From.Y + i.To.Y) / 2));
                }
            }
