...
namespace db
{
    partial class Database
    {
        ...
        public Guild CreateGuild(Account acc, string name)
        {
            ...
            cmd = CreateQuery();
            cmd.CommandText = "INSERT INTO guildboards (guildId, text) VALUES (@id,@empty)";
        ...
        public string GetGuildBoard(Account acc)
        {
            if (acc != null)
            {
                ...
                var cmd = CreateQuery();
                cmd.CommandText = "SELECT * FROM guildboards WHERE guildId=@gid";
        ...
        public string SetGuildBoard(string text, Account acc)
        {
            if (acc != null)
            {
                int gid = acc.Guild.Id;
                var cmd = CreateQuery();
                cmd.CommandText = "UPDATE guildboards SET text=@txt WHERE guildId=@gid";
                cmd.Parameters.AddWithValue("@gid", gid);
