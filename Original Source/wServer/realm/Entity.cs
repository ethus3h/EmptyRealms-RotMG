...
namespace wServer.realm
{
    public class Entity : IBehaviorHost, IProjectileOwner, ICollidable<Entity>
    {
        ...
        public Entity Move(float x, float y)
        {
            ...
            {
                if (this is NPC)
                    (this.ObjectDesc.Enemy ? Owner.EnemiesCollision : Owner.PlayersCollision).Move(this, x, y);
                else
        ...
        public void ImportStats(ObjectStats stat)
        {
            Id = stat.Id;
            if (this is NPC)
                (this.ObjectDesc.Enemy ? Owner.EnemiesCollision : Owner.PlayersCollision).Move(this, stat.Position.X, stat.Position.Y);
            else
                (this is Enemy ? Owner.EnemiesCollision : Owner.PlayersCollision).Move(this, stat.Position.X, stat.Position.Y);
            X = stat.Position.X;
            Y = stat.Position.Y;
